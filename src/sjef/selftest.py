"""Offline controles van de pure logica, gestart via `sjef selftest`."""
...
def run_selftest() -> int:
    ...
    from sjef.planning import matcher, orchestrator, planner
    ...
    check(
        "bulk heeft kcal+eiwit",
        "kcal_per_day" in macros and "protein_per_day" in macros,
    )
    ...
    check(
        "pick_best werkt op geneste respons", nbest is not None and nbest["id"] == "n1"
    )
    check(
        "platte lijst blijft werken",
        len(matcher.flatten_results(results)) == len(results),
    )
    ...
    sl_results = [
        {
            "items": [
                ...
                },
                {
                    ...
                    "name": "Ui-chips",
                    "display_price": 199,
                    "unit_quantity": "150 gram",
                },
                {
                    "id": "duur",
                    "name": "Cadeaupakket ui",
                    "display_price": 9999,
    ...
    kip_cands = [
        {
            "id": "big",
            "name": "Kipfilet 800g",
            "unit_quantity": "800 gram",
            "price_cents": 800,
        },
        {
            "id": "mini",
            "name": "Kipfilet 200g",
            "unit_quantity": "200 gram",
            "price_cents": 379,
        },
    ]
    items_wc = [
        {
            "planned_item": "Kipfilet",
            "zoekterm": "kipfilet",
            "hoeveelheid": "1.4 kg",
            "geschat_aantal": 2,
            "voorraadkast": False,
            "candidates": kip_cands,
        }
    ]
    ...
    check(
        ...
        asm["matched"][0]["voorraadkast"] is False,
    )
    pantry_wc = [
        {
            "planned_item": "Sojasaus",
            "zoekterm": "sojasaus",
            "hoeveelheid": "1 fles",
            "geschat_aantal": 1,
            "voorraadkast": True,
            "candidates": [
                {
                    "id": "soy",
                    "name": "Sojasaus",
                    "unit_quantity": "150 ml",
                    "price_cents": 199,
                }
            ],
        }
    ]
    pasm = matcher.assemble_from_choices(
        pantry_wc, [{"index": 0, "product_id": "soy", "aantal": 1}]
    )
    check(
        "voorraadkast-item gemarkeerd (true)",
        pasm["matched"][0]["voorraadkast"] is True,
    )
    asm_bad = matcher.assemble_from_choices(
        items_wc, [{"index": 0, "product_id": "bestaat-niet", "aantal": 1}]
    )
    check(
        "valt terug op beste kandidaat bij ongeldig id",
        asm_bad["matched"][0]["id"] == "big",
    )
    irrel = [
        {
            "planned_item": "Spirulina",
            "zoekterm": "spirulina",
            "hoeveelheid": "",
            "geschat_aantal": 1,
            "candidates": [
    ...
    asm_none = matcher.assemble_from_choices(
        irrel, [{"index": 0, "product_id": None, "aantal": 1}]
    )
    check(
        "null + irrelevante kandidaat -> unmatched",
        len(asm_none["unmatched"]) == 1 and not asm_none["matched"],
    )
    heur = matcher.assemble_heuristic(items_wc)
    check(
        "heuristische fallback kiest beste kandidaat", heur["matched"][0]["id"] == "big"
    )
    ...
    check(
        "na komma eraf",
        matcher.clean_search_term("Eiwitpoeder (whey, naturel)") == "Eiwitpoeder",
    )
    check(
        "meerwoord blijft",
        matcher.clean_search_term("Rode linzen (voor soep)") == "Rode linzen",
    )

    print("matcher._fallback_terms:")
    check(
        "hoofdwoord als fallback",
        "tomaten" in matcher._fallback_terms("gehakte tomaten"),
    )
    ...
    asm_null = matcher.assemble_from_choices(
        honing_item, [{"index": 0, "product_id": None, "aantal": 1}]
    )
    check(
        "null -> unmatched (geen handzeep als honing)",
        not asm_null["matched"] and len(asm_null["unmatched"]) == 1,
    )
    ...
    check(
        "trim raakt eerst de duurste (kipfilet)",
        any(t["name"] == "Kipfilet" for t in log_),
    )
    check(
        "trim laat goedkoop item (olijfolie) staan",
        any(m["id"] == "p3" for m in trimmed_cart),
    )
    notrim, log0 = matcher.trim_to_budget(cart, max_cents=99999)
    check(
        "geen trim als binnen budget",
        log0 == []
        and matcher.cart_total_cents(notrim) == matcher.cart_total_cents(cart),
    )
    ...
    check(
        "None bij ontbrekende prijs",
        matcher.normalize_candidate({"id": "x", "name": "y"}) is None,
    )
    ...
    check(
        "geschat_aantal opgehoogd naar >=1",
        plan["boodschappenlijst"][0]["geschat_aantal"] == 1,
    )
    ...
    slots = orchestrator.parse_slots(
        {
            "delivery_slots": [
                {
                    "slot_id": "s1",
                    "window_start": "2026-05-28T17:00:00Z",
                    "window_end": "2026-05-28T19:00:00Z",
                }
            ]
        }
    )
    ...
    check(
        "toont ALLE producten (geen afkapping)",
        "Product 49" in text and "nog " not in text,
    )
    ...
    check(
        "elk bericht binnen limiet",
        all(len(m["text"]) <= formatting.TG_LIMIT for m in msgs),
    )
    joined = "\n".join(m["text"] for m in msgs)
    check(
        "alle 50 producten verdeeld over berichten",
        "Product 0" in joined and "Product 49" in joined,
    )
    check("menu-maaltijd zichtbaar in berichten", "Spaghetti bolognese" in joined)
    check(
        "menu gegroepeerd per maaltijdtype", "Ontbijt:" in joined or "Diner:" in joined
    )
    ...
    check(
        "macros tellen ongeveer op tot kcal",
        abs(
            (niels["eiwit_g"] * 4 + niels["vet_g"] * 9 + niels["koolhydraten_g"] * 4)
            - niels["kcal"]
        )
        <= 60,
    )
    ...
    prof = Profiles.load(ROOT / "profiles.example.yaml")
    check("voorbeeld-profiel laadt", prof is not None)
    if prof:
        ctx = prof.planner_context()
        check("twee personen in context", len(ctx["persons"]) == 2)
        check(
            "elke persoon heeft kcal-target",
            all(p["targets"]["kcal"] > 0 for p in ctx["persons"]),
        )
        check("days uit household", ctx["days"] == 7)

    print("planner.build_profiles_prompt:")
    prompt = planner.build_profiles_prompt(
        prof.planner_context(),
        request="extra eiwit",
        budget_target_eur=130,
        cost_conscious=True,
    )
    check(
        "noemt beide personen",
        all(p["name"] in prompt for p in prof.planner_context()["persons"]),
    )
    ...
    from sjef.household.profiles import Profiles as _Prof
    ...
    check(
        "inactieve niet in planner_context",
        [p["name"] for p in pr.planner_context()["persons"]] == ["A"],
    )
    check("ontbrekende vlag = actief (default true)", pr.all_persons[0].active is True)
    check(
        "weekly_need alleen op actieve",
        pr.planner_context()["weekly_need"]["daily_kcal"]
        == pr.persons[0].targets["kcal"],
    )

    print("picnic_client.extract_order_id (geen fout-positief):")
    from sjef.picnic.picnic_client import PicnicClient as _PC

    check(
        "generiek 'shopping_cart' -> None (niet besteld)",
        _PC.extract_order_id({"id": "shopping_cart", "type": "ORDER"}) is None,
    )
    check("leeg/onbekend -> None", _PC.extract_order_id({}) is None)
    check(
        "echt checkout_order_id -> id",
        _PC.extract_order_id({"checkout_order_id": "abc123"}) == "abc123",
    )
    check("echt order_id -> id", _PC.extract_order_id({"order_id": "ord_9"}) == "ord_9")
    check(
        "checkout.id genest -> id",
        _PC.extract_order_id({"checkout": {"id": "co_7"}}) == "co_7",
    )
    check(
        "checkout met placeholder -> None",
        _PC.extract_order_id({"checkout": {"id": "cart"}}) is None,
    )

    print("config dry-run override:")
    from sjef.config import Config as _Cfg

    c_empty = _Cfg(raw={})
    check(
        "zonder safety -> env-default true", c_empty.dry_run(env_default=True) is True
    )
    check(
        "zonder safety -> env-default false",
        c_empty.dry_run(env_default=False) is False,
    )
    c_empty.set_dry_run(False)
    check(
        "config override wint van env-default",
        c_empty.dry_run(env_default=True) is False,
    )
    ...
    check(
        "budget_target_eur is positief getal",
        isinstance(cfg.budget_target_eur, float) and cfg.budget_target_eur > 0,
    )
    ...
    from sjef.picnic import nutrition_lookup as nl
    ...
    from sjef.planning import meal_macros as mm
                ...
                "maaltijden": [
                    {
                        ...
                        "ingredienten": [
                            {
                                "naam": "Magere kwark",
                                "zoekterm": "magere kwark",
                                "porties": [
                                    {"persoon": "Niels", "gram": 200},
                                    {"persoon": "Eline", "gram": 150},
                                ],
                            },
                            {
                                "naam": "Walnoten",
                                "zoekterm": "walnoten",
                                "porties": [
                                    {"persoon": "Niels", "gram": 30},
                                    {"persoon": "Eline", "gram": 0},
                                ],
                            },
                        ],
                    }
    ...
    check(
        "per-persoon Niels kwark (200×56/100=112)",
        ing0["per_persoon"]["Niels"]["kcal"] == 112,
    )
    check(
        "per-persoon Eline kwark (150×56/100=84)",
        ing0["per_persoon"]["Eline"]["kcal"] == 84,
    )
    # Niels: 112 (kwark) + round(30×56/100=17)=17 -> 129 ; Eline: 84 + 0 = 84
    check(
        "maaltijd per persoon Niels",
        meal["per_persoon"]["Niels"]["kcal"] == 112 + round(30 * 56 / 100),
    )
    check("maaltijd per persoon Eline", meal["per_persoon"]["Eline"]["kcal"] == 84)
    check(
        "dagtotaal per persoon gevuld",
        out["dagen"][0]["per_persoon"]["Niels"]["kcal"]
        == meal["per_persoon"]["Niels"]["kcal"],
    )
    ...
    check(
        "legacy gram verdeeld over personen (elk 100)",
    ...
    check(
        "macros kloppen bij override",
        abs(
            (ov["eiwit_g"] * 4 + ov["vet_g"] * 9 + ov["koolhydraten_g"] * 4)
            - ov["kcal"]
        )
        <= 60,
    )
    ...
    if flow.done:
        ...
        check(
            "prefer geparsed als lijst", d["persons"][0]["prefer"] == ["kip", "eieren"]
        )
